// import React from 'react'
// import './Card.css'
// function MultCard() {
//     return (
//         <div className="grid-container">
//             <div className="item1">
//                 <img src="data:image/webp;base64," className="card-img-top" alt="..." />
//                 <div className="card-body">
//                     <h5 className="card-title">Samsung 55" AU7000 UHD 4K Smart TV 55AU7000</h5>
//                     <p className="card-text">R 7999,00</p>
//                     <a href="#" className="btn btn-primary">ADD TO CART</a>
//                 </div>
//             </div>

//             <div className="item2">
//                 <div className="card ">
//                     <img src="https://www.russells.co.za/media/catalog/product/cache/2bc2f148dc23cafaa22d929dc6e18cfe/a/v/av_hometheater_lhd687_basic_350e.jpg" className="card-img-top" alt="..." />
//                     <div className="card-body">
//                         <h5 className="card-title">LG 1250 W Dvd Home Theatre System LHD-687</h5>
//                         <p className="card-text">R7999,00</p>
//                         <a href="#" className="btn btn-primary">ADD TO CART</a>
//                     </div>
//                 </div>
//             </div>
//             <div class="item3">
//                 <div className="card">
//                     <img src="https://encrypted-tbn1.gstatic.com/shopping?q=tbn:ANd9GcT4RPM4CMn77FurkmS7lZ4zYLBC_Ixmv5YmwElcKtzZ-ukX-YgqZilP1j461RHIa25lc3P08vY2SqOruKUMCqYzROseKCjnD3Lsv2zZcCD0QOKWPH7RZrhg&usqp=CAE" className="card-img-top" alt="..." />
//                     <div className="card-body">
//                         <h5 className="card-title">Lenovo Intel Core I5 Laptop </h5>
//                         <p className="card-text">R9999,00</p>
//                         <a href="#" className="btn btn-primary">ADD TO CART</a>
//                     </div>
//                 </div>
//             </div>
//             <div className="item4">
//                 <div className="card">
//                     <img src="https://api-beta-game.walmart.com/medias/media-000000000000826824-1-Default-WF-Default-Product?context=bWFzdGVyfHByb2Nlc3NlZHwxMTM5NXxpbWFnZS9qcGVnfGg4Mi9oMzgvMTAyNzQ4NDA2NDE1NjYvbWVkaWFfMDAwMDAwMDAwMDAwODI2ODI0XzFfRGVmYXVsdC1XRl9EZWZhdWx0LVByb2R1Y3R8NGVmMWZlZDlhMTZiNDRkZTlhOTViYzM4YzE2MTViOTJjZTYzNjBjMGEzZDNhMTYxZjcwMjY0MWY4N2ZhOWE2MA" className="card-img-top" alt="..." />
//                     <div className="card-body">
//                         <h5 className="card-title">Samsung 617L S/S Black Mirror Fridge RS64R53</h5>
//                         <p className="card-text">R26999,00</p>
//                         <a href="#" className="btn btn-primary">ADD TO CART</a>
//                     </div>
//                 </div>
//             </div>
//             <div className="item5">
//                 <div className="card">
//                     <img src="https://assets.mmsrg.com/isr/166325/c1/-/ASSET_MMS_102773003/fee_786_587_png" className="card-img-top" alt="..." />
//                     <div className="card-body">
//                         <h5 className="card-title">Honor x7a 128GB</h5>
//                         <p className="card-text">R8000,00</p>
//                         <a href="#" className="btn btn-primary">ADD TO CART</a>
//                     </div>
//                 </div>
//             </div>
//             <div className="item6">
//                 <div className="card">
//                     <img src="https://www.everyshop.co.za/media/catalog/product/cache/3aaf7ef652111a7c23335511164c8ac8/3/b/3bdecoderwithsatellite_copy_900c.jpg" className="card-img-top" alt="..." />
//                     <div className="card-body">
//                         <h5 className="card-title">DStv Explora Decoder 3B Fully Installed</h5>
//                         <p className="card-text">R2499,00</p>
//                         <a href="#" className="btn btn-primary">ADD TO CART</a>
//                     </div>
//                 </div>
//             </div>
//             <div className="item7">
//                 <div className="card">
//                     <img src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcQnTfFHOsxsy5kb9sCWVYgqJeVNNKmflRHzptsfCYxDV43lE9Alo1m3KcLQdHDa1QoDiQPXNHCYbj0bk3wz6euN6HRx0Jfbjptsyv29dhnkxUAkpjk-eMxqGQ&usqp=CAc" className="card-img-top" alt="..." />
//                     <div className="card-body">
//                         <h5 className="card-title">Julio Tub Chair</h5>
//                         <p className="card-text">R1790,00</p>
//                         <a href="#" className="btn btn-primary">ADD TO CART</a>
//                     </div>
//                 </div>
//             </div>
            
//         </div>
//     )
// }
// export default MultCard;
